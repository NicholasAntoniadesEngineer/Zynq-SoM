...
| OPEN-3 | MED (motor_io) | **3 protection/SI adds — (b) LANDED, (a)+(c) need a verified LCSC only.** KEY: none need the EasyEDA CAD API — these are jellybean packages with STOCK KiCad footprints (no fetch), the LCSC code rides in the value/BOM + `ratings.py`. **(b) ✅ DONE:** 470 µF/35 V bulk **`C976030`** (DMBJ RVT1V471M1010, confirmed on the LCSC page) on the LOAD-side `ESC_VRAIL` (by J3→ESCs — local ESC bulk + stabilises the INA's bus-V node; on `ESC_VRAIL_IN` it overran the schematic router's pin budget for that net), stock `Capacitor_SMD:CP_Elec_10x10.5`, `Device:C_Polarized`. **(a) TODO:** 5 V-rated 8-ch ESD on the ESC PWM lines — **TPD4E05U06** (5.5 V VRWM, ×2, stock `SOT-23-6`); needs the LCSC code + the datasheet channel↔pin map (the lib's 4 V TPD4E02B04 `C106794` would clamp 5 V — do NOT use it). **(c) TODO:** 2–3 W 2512 10 mΩ shunt (stock `R_2512`) to lift the ~7 A ceiling. | (a)/(c): verify the LCSC code on the LCSC product page (WebFetch — the search API is flaky), add with the stock footprint + a `ratings.py` line, instantiate, re-gate. No part fetch needed. |
