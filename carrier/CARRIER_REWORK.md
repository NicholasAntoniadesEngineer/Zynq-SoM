...
| **OPEN-3** | MED (motor_io, part-fetch blocked) | **3 sourced-parts adds — codes being sourced, footprint *fetch* blocked on the EasyEDA CAD API (rate-limiting/CloudFront 5xx).** NOT guessed (LAW 7): (a) a **5 V-rated** output ESD array on the 8 ESC PWM lines (the 3.3 V PESD3V3L4UG would clamp the 5 V PWM) — candidate **TPD4E05U06** (TI, 4-ch, 5.5 V VRWM, ultra-low-cap; ×2 for 8 ch), LCSC code TBD; (b) a **220–470 µF ≥35 V** bulk electrolytic on `ESC_VRAIL_IN` to damp the hot-plug LC ring under the INA3221's 26 V — **VERIFIED `C976030`** (DMBJ RVT1V471M1010, 470 µF/35 V, SMD D10×L10.2 mm, 39 k stock, confirmed on the LCSC product page); (c) a **2–3 W 2512 10 mΩ** shunt + fuse/polyfuse to lift the present ~7 A ceiling (the 1 W RS1 limit), LCSC code TBD. | When the EasyEDA CAD API recovers (it rate-limited this session after the OPEN-2 part adds): `schgen part add C976030` + the ESD/shunt LCSCs, instantiate in motor_pwm/motor_sense, re-gate. The cap is fully sourced — only the footprint/3D pull is blocked. |
