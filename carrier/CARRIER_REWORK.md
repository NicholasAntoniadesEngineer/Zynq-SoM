...
|---|-----|------|--------------|
| ~~OPEN-1~~ ✅ RESOLVED (top silk) | LOW (silk / LAW-1) | **Interior reference designators overlap** — `TP*`, `U6*`, `D6*`, `U12*` etc. overprinted on F.SilkS in the dense diode/IC strings (KiCad auto-ref placement). DONE for the **top** silk: (1) test-point refs hidden (keyed on the `TestPoint` footprint — a TP is identified by the net it probes, its Value); (2) a new `_declutter_refdes` pass (`schgen/generate/pcb.py`) relocates only the refs that actually overlap to the nearest clear spot outside their courtyard, reusing `_place_clear_label` (bounds-aware), top-side only so the F→B mirror never enters; non-colliding refs stay byte-identical. **F.SilkS overlapping ref pairs 74 → 0** (objective census), DRC 0, schematics untouched, render-verified. |
| ~~OPEN-1b~~ ✅ RESOLVED | LOW (silk / LAW-1, bottom) | **Bottom-silk refdes overlap** — DONE, **229 → 0 B.SilkS overlaps**, now ENFORCED. The true count was 229 (the prior 231 used the wrong mirror). Workflow-calibrated the KiCad F→B transform against the renderer: a B.Cu footprint is mirrored, so a child's board position is `fp + R(-frot)·(lx,ly)` (the obvious x-mirror was unfalsifiable — every real B.SilkS ref has lx=0 — and wrong by 6–8 mm for any lx≠0). Three parts: (1) the under-SoM cap grid (uniform ~2 mm pitch, no room for a legible ref at any font) is HIDDEN (`_hide_undersom_bottom_refs`, test-point treatment); (2) `_declutter_refdes` extended to B.SilkS with the mirror forward+inverse, and bottom refs clear only BOTTOM-side silk (the top-side obstacle set was over-constraining the dense banks); (3) `refdes_overlap_gate` composes B.SilkS with the mirror + `enforce_bottom=True` in the build. F.SilkS stays byte-identical (top refs visited first); render-verified; B.Cu-mirror unit test locks the transform. |
...
| OPEN-3 | MED (motor_io) | **2 of 3 protection/SI adds LANDED — none needed the EasyEDA CAD API** (jellybean parts, STOCK KiCad symbol + footprint, LCSC code in the value/BOM + `ratings.py`). **(a) ✅ DONE:** 5 V-rated 8-ch output ESD — two **SRV05-4** arrays (`Power_Protection:SRV05-4` KiCad symbol, stock `SOT-23-6`, **`C2836319`** MSKSEMI 375 k stock; 5 V-line rated so it does NOT clamp the 5 V signals, unlike the spec's 3.3 V part) clamp `ESC_OUT0..7` to +5V/GND at the connector (`motor_pwm`; distributed across 8 nets so no router congestion). **(b) ✅ DONE:** 470 µF/35 V bulk **`C976030`** on the load-side `ESC_VRAIL` (stock `CP_Elec_10x10.5`). **(c) TODO — *design*, not sourcing:** the in-line shunt carries the **aggregate ESC current** (8×~5 A ≈ 40 A); 10 mΩ then = 0.4 V (over the INA3221's ±163 mV range) + 16 W. A higher-current build needs a *value* change (~2 mΩ, e.g. `C494555` LCSR2512FR002, stock `R_2512`) **and** INA-range/scaling rework — a deliberate decision, not a swap. | (c): pick the target max current, size the shunt to the INA range + power, re-scale the firmware. The present 10 mΩ/1 W is correct for the low-current (≤~10 A) bench demo. |
