...
| **OPEN-1b** | LOW (silk / LAW-1, bottom) | **Bottom-silk refdes overlap under the SoM** — the census also found ~232 *B.SilkS* overlapping pairs: the 18 `som_decoupling` bypass caps packed in a tight grid under the SoM footprint (bottom side) overprint each other; there is no room to spread them. Separate from OPEN-1 (which was the top-silk clusters). | Either hide the under-SoM bottom-cap refs (inaccessible under the module, identified by net/value like the TPs) **or** extend `_declutter_refdes` to B.SilkS with a correct F→B local-mirror inverse (the 357/561 bottom footprints the top-only pass deliberately skips). |
