...
| **OPEN-2** | LOW (LAW-6, motor_io) | **XT60 ESC-power connectors edge-flush** — `J37002/J37003` (XT60PW-M) are placed near the E edge + silk-labelled (`ESC PWR IN/OUT`) but not added to `CONN_MATING_FACE`. Their bullet pads give a **+X** mating mouth, and the placer's `connector_edge_rotation` orients only **+Y/−Y** in-plane mouths; the through-hole bullets suggest a **top-entry** part (mates from above → no edge-flush needed, like the servo header). | First confirm the part's entry axis from the 3D model/datasheet. If top-entry: nothing to do (current placement correct). If horizontal edge-mate: extend the placer to orient **+X/−X** mouths (new rows in `_ROT_FACE_*` + the `_mating_face_out_dir` oracle + the placement_mech/connector_model gates), then add `XT60PW-M` to `CONN_MATING_FACE`. |
