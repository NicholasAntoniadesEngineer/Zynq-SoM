"""BIND-PARITY guard for the carrier pmod ADAPTER.

The carrier pmod subsystem is a THIN ADAPTER: it imports the project-agnostic
library subsystem ``subsystems/pmod/`` and BINDS its abstract ports/rails to the
carrier's real net names via the standard ``META`` contract. The library's own
electrical correctness is proven by ``subsystems/pmod/test_pmod.py``; this
...
def test_carrier_real_names_present(adapter):
    ...
    (the bind landed), including the gated rail + the J2 bank-13 host signals."""
    a = adapter.circuit()
...
def test_no_abstract_interface_leak(adapter):
    ...
    library abstract one the META was supposed to rebind (no PMOD*_SIG* leaks)."""
    a = adapter.circuit()
