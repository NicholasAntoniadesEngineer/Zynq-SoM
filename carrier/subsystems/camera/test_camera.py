"""BIND GUARD for the carrier camera ADAPTER (offline, no kicad / no board).

This is a CARRIER-LOCAL bind guard: it proves the thin adapter
(carrier/subsystems/camera/camera.py) binds the project-agnostic library
subsystem (subsystems/camera/) to the carrier's REAL net names byte-for-byte, so
the emitted carrier/schematic/camera.kicad_sch + its golden render stay
unchanged. The library's OWN electrical correctness is proven by
subsystems/camera/test_camera.py — this guard only checks the BIND.
...
from carrier.subsystems.camera import circuit, META
...
def test_carrier_real_net_names_present():
    ...
    (the CAM_* diff pairs + control bus)."""
