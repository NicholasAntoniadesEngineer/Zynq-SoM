...
connector-only protection cell, lightweight. Covers ONLY PORT-ENTRY ESD (the
USBLC6 on the USB2 HS data pair at the receptacle). NO invented electrical
...
refs (usb_jtag_connector.py's J1/U1), carried to board refs by the same per-sheet
band rename the netlist uses.

usb_jtag_connector actives (usb_jtag_connector.py, netlist-verified):
...
USB_JTAG_CONNECTOR IS AN ESD-ONLY CELL. The only active part is the passive
