"""M1: hand-placed RC divider — proves model -> emit -> netlist gate -> ERC.

+3V3 -- R1 -- MID(port) -- R2 -- GND, with C1 from MID to GND.
Everything on the 1.27 mm grid; every wire endpoint EXACTLY on a pin.
Run:  PYTHONPATH=eda/src .venv/bin/python -m schgen.tests.m1_rc
"""
...
from schgen.emit import (HierLabel, Junction, NoConnect, PlacedDesign,
                         PlacedPart, PlacedPower, Wire, emit)
...
def build() -> tuple[Circuit, PlacedDesign, Library]:
    ...
    # pin sanity: compute from the library, never by hand
    pins = {}
    for p in d.parts:
    ...
    d.wires += [
        Wire(96.52, 77.47, 101.6, 77.47),     # PWR_FLAG -> +3V3 node
        Wire(101.6, 85.09, 101.6, 91.44),     # R1.2 -> tap
        Wire(101.6, 91.44, 101.6, 97.79),     # tap -> R2.1
        Wire(101.6, 91.44, 111.76, 91.44),    # tap -> C1.1
        Wire(111.76, 91.44, 116.84, 91.44),   # C1.1 -> label stub
        Wire(101.6, 105.41, 106.68, 105.41),  # GND node -> PWR_FLAG
    ]
