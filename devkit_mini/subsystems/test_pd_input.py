...
NAME = "pd_input"
HERE = Path(__file__).resolve().parent          # devkit_mini/subsystems/ (flat adapter)
...
def _resolve_library_imports() -> None:
    ...
    TOP-LEVEL library (``<repo>/subsystems``), not to ``devkit_mini/subsystems``.

    pytest roots carrier tests at ``carrier/`` (carrier has no ``__init__.py``),
    so ``carrier/`` lands first on sys.path and ``devkit_mini/subsystems/__init__.py``
