...
construction, so every port resolves against `devkit_mini/som_interface.json` and is
consumed by name from the carrier feature sheets (`lcd`, `camera`, `fmc`,
`board_services`, `bringup_rails`).

`som_j3.py` calls the shared generator `devkit_mini/som_conn_gen.py`:
...
```
pytest devkit_mini/subsystems/som_j3/test_som_j3.py
```
