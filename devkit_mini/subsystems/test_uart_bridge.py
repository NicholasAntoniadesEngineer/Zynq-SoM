"""BIND GUARD for the carrier uart_bridge ADAPTER (offline, no kicad / no board).

This is a CARRIER-LOCAL bind guard: it proves the thin adapter
(devkit_mini/subsystems/uart_bridge/uart_bridge.py) binds the project-agnostic
library subsystem (subsystems/uart_bridge/) to the carrier's REAL net names
byte-for-byte (including the UART null-modem crossover that lives in the bind
map), so the emitted carrier/schematic/uart_bridge.kicad_sch + its golden render
stay unchanged. The library's OWN electrical correctness is proven by
subsystems/uart_bridge/test_uart_bridge.py — this guard only checks the BIND.
...
from devkit_mini.subsystems.uart_bridge import circuit, META
...
def test_carrier_real_net_names_present():
    ...
    (including the four crossed UART0 nets)."""
