...
CARRIER_TEMPLATE_DIR = REPO_ROOT / "scripts" / "carrier_template"
...
def _classify_pl_io(n: str) -> dict:
    ...
    if bank == "33":
        if is_diff:
            # First 4 pairs to LVDS LCD, next 4 pairs to HDMI RX, rest to FMC
            return {"destination": "J_LCD_or_HDMIRX", "carrier_signal": n, "interface": "LVDS_LCD/HDMI_RX", "notes": "Bank 33 diff: LCD pairs + HDMI RX TMDS (split assignment)"}
    ...
    if bank == "35":
        ...
        if is_mrcc:
            return {"destination": "J_MRCC_SMA", "carrier_signal": n, "interface": "CLOCK", "notes": "MRCC clock input via SMA"}
        if is_diff:
            return {"destination": "J_FMC.LA12-LA23 or J_CAM", "carrier_signal": n, "interface": "FMC_LA/MIPI", "notes": "Bank 35 LVDS -> FMC LA + MIPI camera (split)"}
