...
The carrier generator works exclusively in KiCad-native millimetres on the
50 mil (1.27 mm) grid. Off-grid coordinates cause silent KiCad connectivity
failures - every coordinate that reaches the emitter is asserted on grid via
``assert_on_grid``.

KiCad schematic space has +Y pointing visually downward on the page. We do
NOT define screen-direction helpers here: with the new hierarchical-block
layout each block builder operates inside its own bounded sheet, and the
``placement.py`` helpers in this package already pick directions relative
to a pin's known side.
"""
...
class Point:
    ...
    Implements ``__iter__`` so it can be spread into APIs that accept
    ``(x, y)`` tuples (notably ``kicad-sch-api``'s position arguments).
    """
