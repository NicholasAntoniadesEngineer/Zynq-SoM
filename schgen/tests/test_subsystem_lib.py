...
def test_bind_renames_diff_pair_complement():
    ...
    assert c.port_type_of("BRD_P").pair_with == "BRD_N"
    assert c.port_type_of("BRD_N").pair_with == "BRD_P"
    assert c.port_type_of("BRD_P").impedance == 100
...
def test_carrier_structure_all_complete():
    ...
    assert res.n_locals == 20, [p.name for p in res.packages if not p.adapter]
    assert len(res.packages) == 37
