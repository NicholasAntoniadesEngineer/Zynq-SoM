...
- ``part.py``        MPN, LCSC id, description, datasheet URL, pin table
...
- ``easyeda_cad.json``  the raw API response (provenance + offline regen
                     via ``--from-json``).
...
def add_part(lcsc_id: str, name: str | None = None,
    ...
    (outdir / "part.py").write_text(gen_part_py(base, info, pins, model_files))
    (outdir / "easyeda_cad.json").write_text(
