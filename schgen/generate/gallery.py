...
def _wired_sheets() -> frozenset[str]:
    p = REPO_ROOT / "carrier" / "project.json"
    if not p.exists():
        return frozenset()
...
def _ratsnest_lines(rel) -> list[str]:
    ...
    board = [(f"carrier/renders/{stem}.png", label)
             for stem, label in RATSNEST_BOARD
             if (RENDERS / f"{stem}.png").exists()]
    if (RATSNEST_DIR / "som.png").exists():
        board.append(("carrier/renders/ratsnest/som.png", "SoM region"))
    ...
    if sheets:
        ...
        cells = []
        for name in sheets:
            png = rel(f"carrier/renders/ratsnest/{name}.png")
...
def _compact_section(readme_dir: Path) -> str:
    ...
    if (RENDERS / f"{RATSNEST_HERO}.png").exists():
        carrier_md = rel("carrier/README.md")
        L += [f'[<img src="{rel(f"carrier/renders/{RATSNEST_HERO}.png")}" '
