...
CatalogPart parse_part_json(const fs::path& path) {
    ...
    const std::string schema = require_string(root, "schema", false, "catalog");
    if (schema != kPartSchema) {
        throw std::runtime_error("catalog: " + path.string()
                                 + " schema must be " + std::string(kPartSchema));
    }
    CatalogPart part;
    part.mpn = require_string(root, "mpn", false, "catalog");
    part.safe_name = require_string(root, "safe_name", false, "catalog");
    part.lcsc = require_string(root, "lcsc", false, "catalog");
    part.description = require_string(root, "description", false, "catalog");
    part.manufacturer = require_string(root, "manufacturer", false, "catalog");
    part.package = require_string(root, "package", false, "catalog");
    part.jlc_class = require_string(root, "jlc_class", true, "catalog");
    part.prefix = require_string(root, "prefix", false, "catalog");
    part.datasheet = require_string(root, "datasheet", false, "catalog");
    part.product_url = require_string(root, "product_url", true, "catalog");
    part.lib_id = require_string(root, "lib_id", false, "catalog");
    part.footprint = require_string(root, "footprint", false, "catalog");
    ...
    for (const JsonNode& pin_node : pins->array_value) {
        ...
        CatalogPin pin;
        pin.number = require_string(pin_node, "num", false, "catalog");
        pin.name = require_string(pin_node, "name", false, "catalog");
        pin.etype = require_string(pin_node, "etype", false, "catalog");
