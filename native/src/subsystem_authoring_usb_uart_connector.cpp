...
// Migrated construction operations from subsystems/usb_uart_connector/usb_uart_connector.py.
CircuitSheetIr usb_uart_connector(const SubsystemMeta& meta, const AuthoringContext& context) {
    CircuitAuthor c("usb_uart_connector", "USB-C UFP console port -> CP2102N", context);
    {
    ...
    {
        AuthoringPort t;
        t.expect = meta.expect_kw("VBUS");
        c.port("VBUS", {"J1.VBUS", "U1.5", "C1.1"}, t, false);
    }
    c.net("GND", {"C1.2"}, std::nullopt);
    c.net("USB_UART_DP_CONN", {"J1.DP1", "J1.DP2", "U1.1"}, std::nullopt);
    c.net("USB_UART_DM_CONN", {"J1.DN1", "J1.DN2", "U1.3"}, std::nullopt);
    {
        AuthoringPort t;
