...
struct ReviewedSources { std::string legacy, native; };
std::string source_comments(const std::string& lines) {
    std::istringstream input(lines);std::string line,out;
    while(std::getline(input,line))out+=" *   "+line+'\n';
    return out;
}
void replace_count(std::string& text,const std::string& old,const std::string& replacement,std::size_t count) {
    std::size_t found=0,position=0;
    while((position=text.find(old,position))!=std::string::npos) {
        text.replace(position,old.size(),replacement);position+=replacement.size();++found;
    }
    require(found==count,"reviewed literal occurrence count changed: "+old);
}
std::string native_notice_expected(std::string expected,const std::string& artifact,const ReviewedSources& sources) {
    // Independent, reviewed deltas applied ONLY to immutable expected bytes,
    // after frozen mutant edits. Never include production templates here, use
    // renderer output as an oracle, strip comments, or normalize actual text.
    if(artifact=="zynq_carrier_contract.h") {
        replace_once(expected,
        ...
        replace_once(expected,sources.legacy,sources.native);
        // The independently frozen missing-power mutant has no rail section.
        const std::string rail="/* ---- Rail bring-up sequence (derived from the power.py regulator     */";
        if(expected.find(rail)!=std::string::npos)replace_once(expected,rail,
            "/* ---- Rail bring-up sequence (derived from power/circuit.json's regulator */");
    } else if(artifact=="BRINGUP.md") {
        replace_once(expected,
        ...
        replace_once(expected,"> narrative context cites its research dossier per section.\n",
            ...
            "`circuit.json`.\n");
        // These are exact authored paths with reviewed occurrence counts, not
        // a blanket .py -> .cpp/.json conversion or a directory discovery gate.
        const std::vector<std::tuple<std::string,std::string,std::size_t>> paths{
            {"carrier/subsystems/pd_input/pd_input.py","carrier/subsystems/pd_input/circuit.json",2},
            {"carrier/subsystems/bringup_rails/bringup_rails.py","carrier/subsystems/bringup_rails/circuit.json",4},
            {"carrier/subsystems/debug_boot/debug_boot.py","carrier/subsystems/debug_boot/circuit.json",2},
            {"carrier/subsystems/usb_pd/usb_pd.py","carrier/subsystems/usb_pd/circuit.json",1},
            {"carrier/subsystems/bringup_en/bringup_en.py","carrier/subsystems/bringup_en/circuit.json",1},
            {"carrier/subsystems/power/power.py","carrier/subsystems/power/circuit.json",1},
            {"carrier/subsystems/power_mon/power_mon.py","carrier/subsystems/power_mon/circuit.json",1},
            {"carrier/subsystems/bringup_modules/bringup_modules.py","carrier/subsystems/bringup_modules/circuit.json",2},
            {"carrier/subsystems/user_io/user_io.py","carrier/subsystems/user_io/circuit.json",1},
            {"carrier/subsystems/bringup_en_modules/bringup_en_modules.py","carrier/subsystems/bringup_en_modules/circuit.json",1},
            {"carrier/subsystems/board_aux/board_aux.py","carrier/subsystems/board_aux/circuit.json",1},
            {"carrier/subsystems/board_services/board_services.py","carrier/subsystems/board_services/circuit.json",1},
            {"carrier/subsystems/board_qwiic/board_qwiic.py","carrier/subsystems/board_qwiic/circuit.json",1}};
        for(const auto& [old,replacement,count]:paths)replace_count(expected,old,replacement,count);
        replace_once(expected,"(`pd_input.py`).","(`pd_input/circuit.json`).");
        replace_once(expected,"so Q1 senses the rail — power.py)","so Q1 senses the rail — power/circuit.json)");
        replace_once(expected,"(`schgen firmware`).","(`native/bin/schgen firmware --project PROJECT_DIR`).");
        replace_once(expected,"(`schgen/bringup_facts.py`)","(`native/src/bringup_facts.cpp`)");
    } else if(artifact=="TEST_PLAN.md") {
        replace_once(expected,
        ...
        replace_once(expected,"`schgen/spice.py`","`native/src/spice.cpp`");
        replace_once(expected,"`schgen/testpoints.py`","`native/src/design_rules.cpp`");
        replace_once(expected,"`schgen/manual.py`","`native/src/firmware_docs_manual.cpp`");
        replace_count(expected,"`schgen/bringup_facts.py`","`native/src/bringup_facts.cpp`",2);
    } else if(artifact=="README.md") {
        replace_once(expected,
        ...
        replace_once(expected,"from `schgen firmware`","from `native/bin/schgen firmware --project PROJECT_DIR`");
        replace_once(expected,"Read from the `power.py` regulator chain","Read from the `power/circuit.json` regulator chain");
    } else if(artifact=="sc_hal_zephyr.c.txt") {
        replace_once(expected," * GENERATED by `schgen scfw` -- DO NOT EDIT.\n",
            " * GENERATED by `native/bin/schgen scfw` (native/src/firmware_docs_scfw.cpp) -- DO NOT EDIT.\n"
            " * regenerate: native/bin/schgen scfw --project PROJECT_DIR\n"
            " * Run from the repository root; replace PROJECT_DIR with your project path.\n");
    } else if(artifact.rfind("sc_",0)==0) {
        replace_once(expected,
            " * GENERATED by `schgen scfw` (schgen/generate/scfw.py) -- DO NOT EDIT.\n"
        ...
        replace_once(expected,
            ...
            " *   ../zynq_carrier_contract.h (the SC hardware contract)\n"+sources.native+
            " *   ../../docs/BRINGUP.md (the staged procedure this automates)\n");
        if(artifact=="sc_tables.h")replace_once(expected,
...
void python_free(const std::string& actual) {
    require(actual.find(".py")==std::string::npos&&actual.find("PYTHONPATH")==std::string::npos&&
            actual.find("python -m")==std::string::npos,"generated artifact still references Python tooling");
}
void source_provenance_mutation(const FirmwareDocsInput& original) {
    auto changed=original;
    changed.firmware_sources={"isolated/custom/circuit.json (mutated canonical source)",
                              "native/src/custom_builder.cpp (mutated native builder)"};
    const auto contract=render_firmware_contract(changed);
    require(contract.find(" *   isolated/custom/circuit.json (mutated canonical source)\n")!=std::string::npos,
            "firmware contract ignored supplied source provenance");
    require(contract.find(" *   native/src/project_authoring_registry.cpp")==std::string::npos,
            "firmware contract retained another project's source provenance");
    std::size_t headers=0;
    for(const auto& artifact:render_scfw(changed)) {
        python_free(artifact.text);
        if(artifact.path=="README.md"||artifact.path=="sc_hal_zephyr.c.txt")continue;
        ++headers;
        require(artifact.text.find(" *   isolated/custom/circuit.json (mutated canonical source)\n")!=std::string::npos&&
                artifact.text.find(" *   native/src/custom_builder.cpp (mutated native builder)\n")!=std::string::npos,
                "SC header ignored supplied source provenance: "+artifact.path);
        require(artifact.text.find(" *   carrier/")==std::string::npos,
                "SC header retained hard-coded carrier source provenance: "+artifact.path);
        require(artifact.text.find("../zynq_carrier_contract.h (the SC hardware contract)")!=std::string::npos&&
                artifact.text.find("../../docs/BRINGUP.md (the staged procedure this automates)")!=std::string::npos,
                "SC header lost its generated contract or staged-procedure dependency");
    }
    require(headers==12,"mutated source provenance must cover all twelve portable SC files");
}
std::string corrected_legacy_output(std::string expected,const std::string& artifact,const ReviewedSources& sources) {
    ...
    return native_notice_expected(std::move(expected),artifact,sources);
}
void exact(const std::string& actual,const std::filesystem::path& expected,const std::filesystem::path& scratch,const ReviewedSources& sources) {
    const auto wanted=corrected_legacy_output(read(expected),expected.filename().string(),sources);
    python_free(actual);
...
void frozen_mutants(const FirmwareDocsInput& original,const std::filesystem::path& dir,const std::filesystem::path& scratch,const ReviewedSources& sources) {
    ...
    for(const auto& test:cases.array_value) {
        ...
        for(const auto& [path,edits]:artifacts) {
            ...
            wanted=corrected_legacy_output(std::move(wanted),std::filesystem::path(path).filename().string(),sources);
            python_free(rendered.at(path));
...
int main(int argc,char** argv) {
    try {
        ...
        for(const std::string name:{"carrier","devkit_mini"}) {
            ...
            const auto paths=resolve_project_paths(root,name);
            const auto reviewed=read(root/"native/tests/data/firmware_provenance"/(name+"_sources.txt"));
            std::string old_sources;for(const auto& source:strings(field(snapshot,"sources")))old_sources+=source+'\n';
            const ReviewedSources sources{source_comments(old_sources),source_comments(reviewed)};
            FirmwareDocsInput in{load_project_circuits(paths),stm32_pin_map(live(field(snapshot,"live")),load_som_interface(paths.som_interface_file)),firmware_sources(paths)};
            std::string actual_sources;for(const auto& source:in.firmware_sources)actual_sources+=source+'\n';
            require(actual_sources==reviewed,"canonical source inventory differs from independently reviewed native fixture");
            ...
            exact(render_firmware_contract(in),dir/"zynq_carrier_contract.h",scratch/name/"zynq_carrier_contract.h",sources);
            const auto sp=spice(field(snapshot,"spice"));const auto probes=pairs(field(snapshot,"probes"));
            exact(render_test_plan(in,sp,probes),dir/"TEST_PLAN.md",scratch/name/"TEST_PLAN.md",sources);
            ...
            exact(render_power_sequence_svg(seq,pr.ok()),dir/"power_sequence.svg",scratch/name/"power_sequence.svg",sources);
            exact(render_power_sequence_svg(seq,false),dir/"power_sequence_fail.svg",scratch/name/"power_sequence_fail.svg",sources);
            ...
            exact(render_test_plan(in,extract_spice_checks(in.sheets),native_probes),dir/"TEST_PLAN.md",scratch/name/"native-TEST_PLAN.md",sources);
            const auto native_power=analyze_power(in.sheets);
            exact(render_power_sequence_svg(build_power_sequence(in.sheets,native_power),native_power.ok()),dir/"power_sequence.svg",scratch/name/"native-power_sequence.svg",sources);
            if(argc==4){
                ...
                require(render_firmware_contract(loaded)==render_firmware_contract(in),"live hardware loader parity");
            }
            if(name=="carrier") {
                exact(render_bringup_manual(in),dir/"BRINGUP.md",scratch/name/"BRINGUP.md",sources);
                const auto docs=render_scfw(in);require(docs.size()==14,"SC file count");for(const auto& f:docs)exact(f.text,dir/"sc"/f.path,scratch/name/"sc"/f.path,sources);
            ...
            frozen_mutants(in,dir,scratch/name/"compiled-frozen-mutations",sources);
