...
void provenance(const std::filesystem::path& root){
    ...
    require(p.providers_complete()&&p.ledger_declarations.size()==83,"current complete provider census");
    for(const auto* board:{"carrier","devkit_mini"}){
        ...
        require(board_policy_reference::field(expected,"ledger").array_value.size()+13==board_policy_reference::field(migrated,"ledger").array_value.size(),"only reviewed replacement and additive ledger population changes");
