...
QuantizationCounts select(const QuantizationCounts& counts,bool new_only){QuantizationCounts out;for(const auto& [name,n]:counts)if(added(name)==new_only)out[name]=n;return out;}
...
void registry(){
    NativeQuantizations q;register_native_quantizations(q);require(q.declarations().size()==26,"twenty original plus six real scalar operations");
