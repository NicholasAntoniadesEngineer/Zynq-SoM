...
void census_file(const std::string& mode,const std::string& ast_path,const std::string& absolute,const std::string& relative,const std::string& output,const std::string& preprocessed){
    const auto input=read_bulk(ast_path),source=read_bulk(absolute);
    const auto start=std::chrono::steady_clock::now();AuditAstProjectionStats stats;
    const auto ast=mode=="raw"?parse_json_text(input,ast_path):parse_audit_ast_projection(input,ast_path,&stats);
...
#endif
        ;
    std::cout<<mode<<" bytes="<<input.size()<<" inner_nodes="<<count<<" retained_values="<<stats.retained_values
        <<" json_values="<<stats.json_values<<" parse_seconds="<<parsed<<" total_seconds="<<seconds(start)
        <<" constants="<<census.constants.size()<<" functions="<<census.functions.size()<<" sites="<<census.quantization.size()<<" peak_rss_bytes="<<peak_bytes<<'\n';
}
}
int main(int argc,char** argv){try{
    ...
    if((argc!=7&&argc!=8)||std::string(argv[1])!="--census"||(std::string(argv[2])!="raw"&&std::string(argv[2])!="projected"))
        throw std::invalid_argument("usage: --manifest | --selftest ROOT | --census raw|projected AST_JSON ABSOLUTE_SOURCE RELATIVE_SOURCE OUTPUT_CENSUS [PREPROCESSED]");
