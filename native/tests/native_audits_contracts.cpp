...
void source_contracts(const fs::path& root,const fs::path& scratch){
    ...
    const auto legalizer=scan_cpp_audit_sources(root,{{"native/src/legalize.cpp"},{"native/src/floorplan_compose.cpp"}},live);
    require(legalizer.quantization.empty(),"legalizer and composer retain no raw quantization after seven scalar extractions");
