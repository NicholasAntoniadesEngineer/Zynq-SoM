...
void registry_contracts(const JsonNode& reference){
    ...
    NativeQuantizations q;register_native_quantizations(q);require(q.declarations().size()==40,"34 prior operations plus six occupancy operations");
...
void source_contracts(const fs::path& root,const fs::path& scratch){
    ...
    const auto census=scan_cpp_audit_sources(root,{{"native/include/schgen/quantize.hpp"},{"native/src/quantize.cpp"},{"native/src/native_audit_quantize.cpp"},{"native/src/precision_ops.cpp"},{"native/src/occupancy_precision.cpp"}},live);
    require(census.constants.size()==8&&census.functions.size()==40,"actual native quantize/precision declarations are scanned");
    ...
    const auto floorplan=scan_cpp_audit_sources(root,{{"native/src/floorplan_build.cpp"},{"native/src/floorplan_ledger.cpp"}},live);
    require(floorplan.quantization.empty(),"floorplan build and ledger contain no remaining raw quantization after exact six scalar extractions");
