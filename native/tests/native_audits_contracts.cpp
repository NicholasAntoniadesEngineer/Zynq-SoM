...
void registry_contracts(const JsonNode& reference){
    ...
    NativeQuantizations q;register_native_quantizations(q);require(q.declarations().size()==34,"twenty original registrations plus six estimate/breathe, two connector and six floorplan operations");
...
void source_contracts(const fs::path& root,const fs::path& scratch){
    ...
    require(census.constants.size()==8&&census.functions.size()==34,"actual native quantize/precision declarations are scanned");
